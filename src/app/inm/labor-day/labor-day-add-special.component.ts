...
@Component({
  selector: 'app-inm-labor-day-add-special',
  templateUrl: 'labor-day-add-special.component.html'
})
export class LaborDayAddSpecialComponent implements OnInit {
  ...
  loadInmateLaborDays() {
    ...
    this.laborDayService.getAvailablePeriodSpecialInmateLabors(this.args).subscribe({
      next: (responseData) => {
  ...
  saveInmateLaborDays() {
    ...
    this.laborDayService.savePeriodSpecialLaborDays(this.inmateLaborDays).subscribe({
      next: (responseData) => {
        ...
        this.router.navigate(['/']).then(() => { this.router.navigate(['/inm/laborday/addspecial']); });
